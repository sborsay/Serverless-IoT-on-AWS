...
for (( i = 1; i <= $iterations; i++)) {
  ...
  humidity=$(( 50 + $RANDOM % 40 ))#  Adjecsted for AWS CLI Version 2, Stephen Borsay
#  code found at: https://aws.amazon.com/blogs/iot/integrating-iot-data-with-your-data-lake-with-new-aws-iot-analytics-features/
#  changed 1000 iterations to 10
...
for (( i = 1; i <= $iterations; i++)) {
 ...
 aws2 iot-data publish --topic "$mqtttopic" --cli-binary-format raw-in-base64-out --payload "{\"temperature\":$temperature,\"humidity\":$humidity}" --profile "$profile" --region "$region"
 ...
 #use below for AWS CLI V2
 aws iot-data publish --topic "$mqtttopic" --cli-binary-format raw-in-base64-out --payload "{\"temprature\":$TEMP,\"humidity\":$HUMIDITY,}" --profile "$profile" --region "$region"
