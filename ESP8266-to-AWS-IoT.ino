...
const char* awsEndpoint = "a32qaa131oyees-ats.iot.us-west-2.amazonaws.com";
...
//-----BEGIN CERTIFICATE-----
"XXXXXjCCAkKgAwIBAgIVAI7n0dti6sMuimCVANnjs+8i8TUiMA0GCSqGSIb3DQEB" \
...
"eyQGw4Qn+O3K2I+nDS3+u/ES6xmesw0O1U3nQW7/9uZs/Eh7e8gh2Zble+3CdZbK" \/* ESP8266 AWS IoT
 *  
 * Simplest possible example (that I could come up with) of using an ESP8266 with AWS IoT.
 * No messing with openssl or spiffs just regular pubsub and certificates in string constants
 * 
...
const String privatePemKey = \
//-----BEGIN RSA PRIVATE KEY-----
"XXXXXAIBAAKCAQEA0UlgP9sUoPzmDjjOndrJiSBJ91SIJqghJq4Nfur3aCgS8YDI" \
...
//choose your own number of variables and data types, but avoid data buffer size overload
  ...
  */
  ...
  char fakeData[100];
  float t =  random(30,110); //fake number range, adjust as you like
  float h =  random(40,100);
 // sprintf(fakeData,  "{\"uptime\":%lu,\"temp\":%f,\"humid\":%f}", millis() / 1000, var1, var2);
 
  snprintf(fakeData, sizeof(fakeData),"{\"uptime\":%lu,\"temp\":%f,\"humid\":%f}", millis() / 1000, t, h);

  

  if (millis() - lastPublish > 10000) {
    String msg = String("Hello from ESP8266: ") + ++msgCount;
    //add json as string then let c_string deal with it
   // pubSubClient.publish("outTopic", msg.c_str());
   pubSubClient.publish("outTopic", fakeData);
  // pubSubClient.publish("outTopic", inputx);
//    Serial.print("Published: "); Serial.println(inputx);
...
void setCurrentTime() {
  configTime(3 * 3600, 0, "1.north-america.pool.ntp.org"/*"pool.ntp.org" */, "time.nist.gov");
